...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!label) return {};
  return {
    title: `${label} — Directorio Argentina e Irlanda 2026 | VoiceFleet`,
    description: `Encontra los mejores ${label.toLowerCase()} en Argentina e Irlanda.`,
  };
}
            ...
            {cities.map(c => (
              <Link key={c.citySlug} href={`/es/directorio/${vertical}/${c.citySlug}`} className="group">
                <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6 hover:border-blue-500/50 transition-all">
                  ...
                  <p className="text-slate-400 mt-1">{c.count} negocios</p>
                </div>
              </Link>
            ))}
          </div>
