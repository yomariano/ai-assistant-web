...
export const metadata: Metadata = generatePageMetadata({
  title: "Blog - AI Voice Agent Insights for Australia | VoiceFleet",
  description:
    "Insights on AI voice agents, customer service automation, and scaling support operations for Australian businesses using VoiceFleet.",
  path: "/au/blog",
});
...
export default async function AustraliaBlogPage() {
  const posts = await getBlogPosts({ limit: 1000, language: "en" });
        ...
        <section className="relative overflow-hidden border-b border-stone-200/80 bg-[radial-gradient(circle_at_top_left,rgba(219,234,254,0.8),transparent_36%),radial-gradient(circle_at_top_right,rgba(226,232,240,0.75),transparent_34%),linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] pt-28 pb-16">
          <div className="mx-auto max-w-6xl px-6">
            <span className="inline-flex items-center rounded-full border border-stone-200 bg-white/85 px-4 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-stone-600 shadow-sm">
              VoiceFleet Australia Journal
            </span>
            <h1 className="mt-6 max-w-4xl font-heading text-5xl font-extrabold tracking-[-0.04em] text-stone-950 md:text-7xl">
              Practical notes on AI phone operations for Australian teams
            </h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-stone-600 md:text-xl">
              Editorial analysis on AI receptionists, missed-call economics, local lead capture, and how Australian service businesses automate phone support without losing quality.
            </p>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-6xl px-6 pt-14">
          {posts.length === 0 ? (
            <div className="rounded-[2rem] border border-stone-200 bg-white px-8 py-16 text-center shadow-[0_24px_60px_rgba(15,23,42,0.06)]">
              <p className="text-lg text-stone-600">No blog posts yet. Check back soon.</p>
            </div>
          ) : (
            <>
              {featuredPost && (
                <Link
                  href={`/au/blog/${featuredPost.slug}`}
                  className="group block"
                  data-umami-event="blog_article_click"
                  data-umami-event-source="blog_index_featured_au"
                      ...
                      <div className="mt-10 space-y-4">
                        <div className="flex flex-wrap items-center gap-3 text-sm text-stone-500">
                          ...
                          <time dateTime={getBlogDisplayDate(featuredPost) || undefined}>
                            {formatBlogDate(getBlogDisplayDate(featuredPost), "en-AU", "long") || "Draft"}
                          </time>
                          <span>&middot;</span>
              ...
              {otherPosts.length > 0 && (
                <div className="mt-14">
                  <div className="mb-8 flex items-center justify-between gap-4">
                    ...
                    <p className="text-sm text-stone-500">{posts.length} published articles</p>
                  </div>
                  ...
                  <div className="grid gap-6 md:grid-cols-2">
                    {otherPosts.map((post) => (
                      <Link
                        key={post.id}
                        href={`/au/blog/${post.slug}`}
                        className="group block"
                        data-umami-event="blog_article_click"
                        data-umami-event-source="blog_index_grid_au"
                          ...
                          <div className="flex flex-1 flex-col p-7">
                            ...
                            <div className="mt-6 flex items-center justify-between gap-4 border-t border-stone-100 pt-5 text-sm text-stone-500">
                              ...
                              <time dateTime={getBlogDisplayDate(post) || undefined}>
                                {formatBlogDate(getBlogDisplayDate(post), "en-AU", "short") || "Draft"}
                              </time>
                            </div>
                          </div>
                        </article>
