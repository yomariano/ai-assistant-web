/**
 * VoiceFleet SEO - Google News RSS Integration
 * Fetches industry-related news for content freshness
 */
...
export async function fetchIndustryNews(
  ...
  try {
    ...
    const response = await fetch(rssUrl, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; VoiceFleetBot/1.0)',
      },
    });
...
function buildSearchQuery(industry: string): string {
  ...
  // Add context for better results - focus on phone/voice/AI trends
  return `${cleanIndustry} AI automation OR ${cleanIndustry} phone technology OR ${cleanIndustry} business trends`;
}
